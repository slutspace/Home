...
const navItems: {
  ...
  { href: '/profile', label: 'Overview', icon: UserCircleIcon, match: 'exact' },
...
function linkActive(pathname: string, href: string, match: 'exact' | 'prefix' = 'prefix') {
          ...
          <nav className="lg:w-56 flex-shrink-0">
            <div className="rounded-xl border border-gray-700 bg-gray-800/50 p-2 space-y-1">
              {navItems.map(({ href, label, icon: Icon, match }) => {
                const active = linkActive(pathname, href, match ?? 'prefix')
                return (
                  <Link
                    key={href}
                    href={href}
